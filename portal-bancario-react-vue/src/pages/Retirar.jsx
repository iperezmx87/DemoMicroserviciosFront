...
function Retirar() {
  ...
  const handleDepositar = async (e) => {
    ...
    if (!monto || monto <= 0) {
      setError(
        "Por favor, ingresa un monto válido para retirar. El monto debe ser mayor a cero.",
      );
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:8070/api/Cuentas/${cuentaId}/retirar`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        throw new Error(data.error || "Error al realizar el retiro.");
      }

      setSuccess(
        "¡Retiro realizado con éxito! El saldo se actualizará en breve.",
      );
      ...
      <Container maxWidth="sm" sx={{ mt: 6 }}>
        <Box sx={{ mb: 4 }}>
          ...
          <Typography variant="subtitle1" color="text.secondary" sx={{ mt: 2 }}>
            Ingresa el monto que deseas retirar de tu cuenta.
            <br />
